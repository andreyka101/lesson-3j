...
export default class LiveBackgroundMod {
  ...
  start(){
    ...
    console.log(Math.round(Math.random() * (300 - 30) + 30));
    for (let i of this.#nameId){
      let element = document.querySelector(`#backgroundSVG_${i}`) as HTMLElement
      element.style.cssText += `
  fill : #51EB00;
  width: ${Math.round(Math.random() * (300 - 30) + 30)}px;
  height: ${Math.round(Math.random() * (300 - 30) + 30)}px;
  top: ${(Math.random() * (92 - -5) + -5).toFixed(2)}%;
  left: ${(Math.random() * (92 - -5) + -5).toFixed(2)}%;
  filter: blur(2px);
  position: fixed;
  `
    }
    ...
    top: ${(Math.random() * (92 - -5) + -5).toFixed(2)}%;
    left: ${(Math.random() * (92 - -5) + -5).toFixed(2)}%;
    filter: blur(${Math.round(Math.random() * (35 - 9) + 9)}px);
    `
      }
