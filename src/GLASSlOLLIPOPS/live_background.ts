...
export default class LiveBackgroundMod {
  ...
  #arraySVG = [] as Array<string>
  #nameIdDiv = [0, 1, 2]
  /** 
   *  type: method([string,string,string,...])
   *  при вызове появляются фигуры, которые случайно перемешаются,
   * их можно заменить на свои svg без цвета принимает массив (не обязательно)
  */
 startAnimationClassic(a:Array<string> = ['0']) {
  if (a[0]!='0'){
    for(let i in a){
